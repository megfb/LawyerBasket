...
namespace LawyerBasket.PostService.Application.CommandHandlers
{
  public class CreatePostCommandHandler : IRequestHandler<CreatePostCommand, ApiResult<PostDto>>
  {
    ...
    public CreatePostCommandHandler(ICurrentUserService currentUserService,IMapper mapper, ILogger<CreatePostCommandHandler> logger, IPostRepository postRepository, IUnitOfWork unitOfWork)
    {
      _logger = logger;
      _mapper = mapper;
