...
export class PostService {
  ...
  private apiUrl = 'https://localhost:7254/api/Post'; // PostService API URL
  ...
  createComment(postId: string, text: string): Observable<ApiResult<CommentDto>> {
    return this.http.post<ApiResult<CommentDto>>(`https://localhost:7254/api/Comment/CreateComment`, {
      postId: postId,
      text: text
    });
  }
  ...
  removeComment(postId: string, commentId: string): Observable<ApiResult<any>> {
    return this.http.request<ApiResult<any>>('DELETE', `https://localhost:7254/api/Comment/RemoveComment/${commentId}`, {
      body: {
        postId: postId,
        commentId: commentId
      }
